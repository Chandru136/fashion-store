...
async function runCheckout(mode){
 let checks=0,verification=0;
 const actions={preparePaymentAction:async()=>({success:true,checkout:{paid:false,mock:false,keyId:'test',gatewayOrderId:'order_fixture',amount:10000,currency:'INR',name:'Test',contact:''}}),
 ...
 const window={Razorpay:class{constructor(options){this.options=options;}open(){if(mode==='dismiss')void this.options.modal.ondismiss();else void this.options.handler({razorpay_order_id:'order_fixture',razorpay_payment_id:'pay_fixture',razorpay_signature:'a'.repeat(64)});}}};
 ...
 assert.equal(await client.openOrderPayment('cmu2giz3b0003gaygsyuzj0t8'),true);
 assert.equal(verification,mode==='dismiss'?0:1);
}
...
test('capture updates payment and order atomically; authorization alone stays pending',async()=>{
 ...
 const tx={$executeRaw:async()=>{},payment:{findUnique:async()=>record,findUniqueOrThrow:async()=>({...record,status:state.payment,order:{status:state.fulfilment,paymentStatus:state.order,expiresAt:null}}),update:async({data})=>{state.payment=data.status;}},order:{update:async({data})=>{state.order=data.paymentStatus;state.fulfilment=data.status;}}};
 const service=load('lib/payments/checkout.service.ts',{'./lifecycle.service':{},'@/lib/db':{prisma:{$transaction:async f=>f(tx)}},zod:require('zod'),'./razorpay':{}});
 const payment={id:'pay_fixture',order_id:'order_fixture',amount:10000,currency:'INR',status:'authorized',captured:false};
 assert.equal(await service.applyPayment(payment),'PENDING');assert.equal(state.payment,'PENDING');
